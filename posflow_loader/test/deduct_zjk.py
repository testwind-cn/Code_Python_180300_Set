...
def run_hive(conf: ConfigData, the_date: str):
    ...
    sql = """
    --deduct sql created by Zhaohu on 14-Nov-2018 am 10:49
use loginfo
"""
    print("OK" + "  " + sql + "\n")
    cur.execute(sql)  # , async=True)

    sql = "set mapreduce.job.queuename = deduct_zjk_mysql;"
    print("OK" + "  " + sql + "\n")
    cur.execute(sql)  # , async=True)
    ...
    sql2 = """
    select distinct 
case  when creq.prod_code='8004000001' then '张家口银行' else '' end as `产品类别`,
...
where creq.prod_code='8004000001'
and from_unixtime(unix_timestamp(deductall.CURR_DATE,'yyyymmdd'))>=date_add(current_date(),-5)
    ...
    name = '/home/data/deduct/deduct_zjk_late5_'+StrTool.get_the_date_str('',-1)+'.xlsx'
